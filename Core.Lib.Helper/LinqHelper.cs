...
namespace Core.Lib.Reflections
{
    ...
    public static class LinqHelper
    {
        ...
        [DebuggerHidden]
        public static TDelegate Reduce<TDelegate>(
          this IEnumerable<TDelegate> delegates,
          Func<TDelegate, TDelegate, TDelegate> reduce)
          where TDelegate : Delegate
          => delegates.Aggregate(reduce);

        [DebuggerHidden]
        public static IEnumerable<TTo> Map<TFrom, TTo>(
            this IEnumerable<TFrom> delegates,
            Func<TFrom, TTo> select)
            where TFrom : Delegate
            where TTo : Delegate
            => delegates.Select(select);

        [DebuggerHidden]
        public static IEnumerable<TResult> Each<TDelegate, TResult>(
            this IEnumerable<IEnumerable<TDelegate>> delegates, Func<TDelegate, TResult> selector)
            where TDelegate : Delegate
            where TResult : Delegate
           => delegates.SelectMany(f => f.Select(selector));
    }
}
