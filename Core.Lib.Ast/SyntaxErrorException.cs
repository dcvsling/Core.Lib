namespace Core.Lib.Ast.Abstractions
{
    ...
    {
        public SyntaxErrorException(string description) : base(description)
        {

        }

        public SyntaxErrorException(string description, string source, Location location)
            : base(ComposeMessage(description, source, location))
        {
        }

        private static string ComposeMessage(string description, string source, Location location)
        {
            return $"Syntax Error GraphQL ({location.ToString()}) {description}" +
                "\n" + HighlightSourceAtLocation(source, location);
        }

        private static string HighlightSourceAtLocation(string source, Location location)
        {
